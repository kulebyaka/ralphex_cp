...
- [x] Save fixtures to `pkg/executor/testdata/copilot_fixtures/` — saved with_signal.jsonl (24 lines) and error_exit_notes.md (documentation of error behavior since no JSONL is produced for CLI errors)
...
- [x] Validate all fixture files are valid JSONL: `for f in pkg/executor/testdata/copilot_fixtures/*.jsonl; do jq empty < "$f" 2>&1 || echo "INVALID: $f"; done` — all 4 JSONL fixtures valid (simple_text 36, simple_text_gpt 10, tool_use 118, with_signal 24); error_exit renamed to .md since it contained no parseable JSONL events

...
func TestOptions_Validate(t *testing.T) {
	tests := []struct {
		...
		{"valid model claude-opus-4.6", Options{Model: "claude-opus-4.6"}, ""},
		{"valid model claude-sonnet-4.6", Options{Model: "claude-sonnet-4.6"}, ""},
		{"valid model claude-haiku-4.5", Options{Model: "claude-haiku-4.5"}, ""},
		{"valid model gpt-5.2-codex", Options{Model: "gpt-5.2-codex"}, ""},
		{"old short keyword rejected", Options{Model: "sonnet"}, `unknown model "sonnet", must be one of:`},
		{"unknown model", Options{Model: "gpt-5"}, `unknown model "gpt-5", must be one of:`},
		{"agent type not validated", Options{AgentType: "anything-goes"}, ""},
		{"unknown model with agent", Options{Model: "bad", AgentType: "reviewer"}, `unknown model "bad", must be one of:`},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warnings := tt.opts.Validate()
			if tt.wantContains == "" {
